...
<div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8"
    ...
    <div class="text-center mb-12">
        ...
        <h1 class="text-4xl md:text-6xl font-bold gradient-text mb-4 leading-tight">
            Perjalanan Belanjamu
        </h1>
        <p class="text-lg text-gray-600 max-w-2xl mx-auto">
            Lihat semua transaksi dan berikan rating untuk produk yang sudah kamu beli
        </p>
    </div>
        ...
        <div class="flex flex-wrap gap-4 justify-center md:justify-start items-center">
            ...
            <div class="flex flex-wrap gap-3">
                <button
                    ...
                    :class="{ 'gradient-button text-white shadow-lg': activeFilter === 'all', 'bg-white/70 text-gray-700 hover:bg-white border border-pink-200': activeFilter !== 'all' }"
                    class="filter-button px-6 py-3 rounded-full font-bold text-sm backdrop-blur-sm"
                >
                ...
                <button
                    ...
                    :class="{ 'bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg': activeFilter === '{{ \App\Models\Order::STATUS_COMPLETED }}', 'bg-white/70 text-gray-700 hover:bg-white border border-green-200': activeFilter !== '{{ \App\Models\Order::STATUS_COMPLETED }}' }"
                    class="filter-button px-6 py-3 rounded-full font-bold text-sm backdrop-blur-sm"
                >
                ...
                <button
                    ...
                    :class="{ 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-lg': activeFilter === '{{ \App\Models\Order::STATUS_SHIPPED }}', 'bg-white/70 text-gray-700 hover:bg-white border border-blue-200': activeFilter !== '{{ \App\Models\Order::STATUS_SHIPPED }}' }"
                    class="filter-button px-6 py-3 rounded-full font-bold text-sm backdrop-blur-sm"
                >
                ...
                <button
                    ...
                    :class="{ 'bg-gradient-to-r from-orange-500 to-red-500 text-white shadow-lg': activeFilter === '{{ \App\Models\Order::STATUS_PROCESSING }}', 'bg-white/70 text-gray-700 hover:bg-white border border-orange-200': activeFilter !== '{{ \App\Models\Order::STATUS_PROCESSING }}' }"
                    class="filter-button px-6 py-3 rounded-full font-bold text-sm backdrop-blur-sm"
                >
                ...
                <button
                    ...
                    :class="{ 'bg-gradient-to-r from-yellow-500 to-orange-500 text-white shadow-lg': activeFilter === '{{ \App\Models\Order::STATUS_PENDING }}', 'bg-white/70 text-gray-700 hover:bg-white border border-yellow-200': activeFilter !== '{{ \App\Models\Order::STATUS_PENDING }}' }"
                    class="filter-button px-6 py-3 rounded-full font-bold text-sm backdrop-blur-sm"
                >
                ...
                <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                    <div class="space-y-3">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <div class="text-sm font-bold gradient-text uppercase tracking-wide">
                                    ID Pesanan: #{{ $order->id }}
                                </div>
                            ...
                            @php
                                ...
                                switch($order->status) {
                                    case \App\Models\Order::STATUS_COMPLETED:
                                        $statusClass = 'from-green-400 to-emerald-500';
                                        $statusIcon = 'fa-check-circle';
                                        break;
                                    case \App\Models\Order::STATUS_SHIPPED:
                                        $statusClass = 'from-blue-400 to-indigo-500';
                                        $statusIcon = 'fa-truck';
                                        break;
                                    case \App\Models\Order::STATUS_PROCESSING:
                                        $statusClass = 'from-orange-400 to-red-500';
                                        $statusIcon = 'fa-cog';
                                        break;
                                    case \App\Models\Order::STATUS_PENDING:
                                        ...
                                        break;
                                    default:
                                        $statusClass = 'from-gray-400 to-gray-500';
                                ...
                                <a href="{{ route('user.products.show', $item->product->id) }}" class="flex items-center space-x-6 flex-grow group cursor-pointer">
                                    <div class="relative">
                                        <img src="{{ asset('storage/' . $item->product->image) }}"
                                             ...
                                             class="w-24 h-24 object-cover rounded-2xl shadow-lg border-2 border-white transform transition-all duration-300 group-hover:scale-110">
                                    ...
                                    <div class="flex-grow">
                                        <h4 class="text-gray-900 font-bold text-xl group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-pink-600 group-hover:to-purple-600 group-hover:bg-clip-text transition-all duration-300">
                                            {{ $item->product->name }}
                                        </h4>
                                        <div class="flex items-center space-x-4 mt-2">
                                            <span class="bg-gradient-to-r from-pink-100 to-purple-100 text-pink-600 px-3 py-1 rounded-full text-sm font-bold">
                                                {{ $item->quantity }} unit
                                            </span>
                                ...
                                <div class="flex-shrink-0">
                                    ...
                                    @elseif($order->status === \App\Models\Order::STATUS_COMPLETED)
                                        <button type="button"
                                                ...
                                                class="gradient-button text-white px-6 py-3 rounded-2xl font-bold shadow-lg hover:shadow-xl transition-all duration-300 flex items-center space-x-2">
    ...
    @empty
        <div class="empty-state rounded-3xl shadow-2xl">
            <div class="text-center py-20 px-8">
                ...
                <h3 class="text-4xl font-bold gradient-text mb-6">
                    Belum Ada Riwayat Pembelian
                </h3>
                <p class="text-gray-600 max-w-xl mx-auto mb-8 text-lg leading-relaxed">
                    Waktunya memulai petualangan belanja yang menakjubkan! Jelajahi koleksi produk pilihan kami dan ciptakan riwayat belanja yang tak terlupakan.
                </p>
    ...
    {{-- ## TAMBAHAN DI SINI: Link Paginasi ## --}}
    <div class="mt-8">
        {{ $orders->links() }}
    </div>
